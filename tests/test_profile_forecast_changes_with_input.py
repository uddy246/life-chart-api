...
def _top_window_ids(response_json: dict) -> list[str]:
    return [window.get("windowId", "") for window in response_json.get("topWindows", [])]


def test_profile_forecast_changes_with_input():
    params_a = {
        "name": "Example Person",
        "date": "1999-02-26",
        "time": "14:00:00",
    ...
    response_a = _get_forecast(params_a)
    response_b = _get_forecast(params_b)

    assert _top_window_ids(response_a) != _top_window_ids(response_b)

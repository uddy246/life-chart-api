...
def test_narrative_example_validates_against_schema():
    base = Path(__file__).resolve().parents[1]
    schema_path = (
        base / "src" / "life_chart_api" / "schemas" / "narrative" / "narrative_response.schema.json"
    )
    example_path = (
        base / "src" / "life_chart_api" / "schemas" / "examples" / "narrative" / "narrative_response.example.json"
    )

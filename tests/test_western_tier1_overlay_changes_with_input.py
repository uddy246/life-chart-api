...
def _extract_signs(western: dict) -> dict[str, str | None]:
    signs: dict[str, str | None] = {"sun": None, "moon": None, "ascendant": None}
    identity = western.get("identity", {})
    if isinstance(identity, dict):
        for key, target in (("sunSign", "sun"), ("moonSign", "moon"), ("ascendant", "ascendant")):
            placement = identity.get(key)
            if isinstance(placement, dict) and isinstance(placement.get("sign"), str):
                signs[target] = placement["sign"]
    planets = western.get("planets", [])
    if isinstance(planets, list):
        for planet in planets:
            if isinstance(planet, dict) and isinstance(planet.get("sign"), str):
                if planet.get("planet") == "sun" and signs["sun"] is None:
                    signs["sun"] = planet["sign"]
                if planet.get("planet") == "moon" and signs["moon"] is None:
                    signs["moon"] = planet["sign"]
    return signs
...
def test_western_tier1_overlay_changes_with_input():
    ...
    western_a = response_a.get("systems", {}).get("western", {})
    western_b = response_b.get("systems", {}).get("western", {})
    signs_a = _extract_signs(western_a)
    signs_b = _extract_signs(western_b)

    assert signs_a["sun"] is not None
    assert signs_b["sun"] is not None
    assert signs_a["sun"] != signs_b["sun"]

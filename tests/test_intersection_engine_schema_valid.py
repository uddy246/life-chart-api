...
def test_intersection_engine_schema_valid():
    payload = {
        "name": "Example Person",
        "birth": {
            "date": "1999-02-26",
    ...
    intersection = response_json.get("intersection", {})
    assert isinstance(intersection.get("convergences"), list)
    assert isinstance(intersection.get("divergences"), list)
    assert isinstance(intersection.get("bridgeTags"), list)
    summary = intersection.get("summary", {})
    assert isinstance(summary.get("headline"), str)
    assert isinstance(summary.get("notes"), list)
    assert isinstance(summary.get("nextActions"), list)

...
def _planet_signs(western: dict) -> dict[str, str | None]:
    signs = {key: None for key in ("mercury", "venus", "mars", "saturn")}
    ...
    for planet in planets:
        ...
        if name in signs and isinstance(sign, str):
            signs[name] = sign
    return signs
...
def test_western_tier2_overlay_structured_fields():
    ...
    signs_a = _planet_signs(western_a)
    signs_b = _planet_signs(western_b)

    assert any(signs_a.values())
    assert any(signs_b.values())
    assert any(
        signs_a[key] != signs_b[key]
        for key in signs_a
        if signs_a[key] is not None and signs_b[key] is not None
    )
